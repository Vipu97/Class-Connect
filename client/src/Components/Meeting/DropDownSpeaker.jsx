...
import test_sound from '../../assets/sounds/test_sound.mp3';
        ...
        {({ open }) => (
          <>
            <Popover.Button
              ...
              group inline-flex items-center rounded-md px-1 py-1 w-60 xxs:w-72 sm:w-48 text-base font-normal
              ${!isMicrophonePermissionAllowed ? "opacity-50" : ""}`}
            >
              <DropSpeaker fillColor={isHovered || open ? "#FFF" : "#B4B4B4"} />
              <span className=" overflow-hidden whitespace-nowrap overflow-ellipsis w-48 ml-3">
                {isMicrophonePermissionAllowed ? selectedSpeaker?.label : "Permission Needed"}
              </span>
              <ChevronDownIcon
                className={`${open ? 'text-orange-300' : 'text-orange-300/70'}
                ml-2 h-5 w-5 transition duration-150 ease-in-out group-hover:text-orange-300/80 mt-1`}
                aria-hidden="true"
              />
            </Popover.Button>
            <Transition
            ...
            >
              <Popover.Panel className="absolute bottom-full right-[-50px] z-10 mt-3 w-72 px-4 sm:px-0 pb-2">
